  ...
  b0 ~ dunif(0.00000001, 10) # Prior for mean of logit survival
  b1 ~ dunif(0, 10)# Prior for slope parameter
  db0 ~ dunif(0, 5) # Prior for mean of logit survival
  ...
  for (i in 1:nind){
    for (t in f[i]:(n.occasions-1)){
         phi[i,t] <-  exp((age[i,t]/b1)^b0-(db1*((age[i,t+1]/b1)^b0+c)+db0*(exp(db1)-1)*exp(-age[i,t+1]*db1))/db1)
         p[i,t] <- mean.p
    } #t
  } #i
  ...
  for(age in 0:7){
    phi.age[age+1]  <-  exp((age/b1)^b0-(db1*(((age+1)/b1)^b0+c)+db0*(exp(db1)-1)*exp(-(age+1)*db1))/db1)
    instantMorta.age[age+1]  <- (b0*(age/b1)^(b0-1))/b1+c+db0*exp(-db1*age)
   cumulSurv.age[age+1] <-  exp(-age^b0*(1/b1)^b0)
  }
