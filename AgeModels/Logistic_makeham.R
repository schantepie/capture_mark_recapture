  ...
  for (i in 1:nind){
    for (t in f[i]:(n.occasions-1)){
         phi[i,t] <-   exp(-((exp(1)*(log(b0*s*exp((age[i,t+1])*b1)+exp(1)*b1)-log(b0*s*exp(age[i,t]*b1)+exp(1)*b1)))/s)-c)
         p[i,t] <- mean.p
    } #t
  } #i
  ...
  for(age in 0:7){
    phi.age[age+1]  <-exp(-((exp(1)*(log(b0*s*exp((age+1)*b1)+exp(1)*b1)-log(b0*s*exp(age*b1)+exp(1)*b1)))/s)-c)
    instantMorta.age[age+1]  <- (b0*exp(b1*age))/((b0*s*exp(b1*age-1))/b1+1)+c
    cumulSurv.age[age+1] <- exp(-((exp(1)*(log(b0*s*exp((age+1)*b1)+exp(1)*b1)-log(b0*s+exp(1)*b1)))/s)-age*c-c)
  }
