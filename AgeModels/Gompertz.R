  ...
  for (i in 1:nind){
    for (t in f[i]:(n.occasions-1)){
         phi[i,t] <-  exp((b0/b1) *(exp(age[i,t]*b1)-exp((age[i,t+1])*b1)))
         p[i,t] <- mean.p
    } #t
  } #i
  ...
  for(aging in 0:7){
    phi.age[aging+1]  <- exp((b0/b1) *(exp(aging*b1)-exp((aging+1)*b1)))
    instantMorta.age[aging+1]  <- b0*exp(b1*aging)
    cumulSurv.age[aging+1] <- exp((b0/b1) *(1-exp((aging)*b1)))
  }
